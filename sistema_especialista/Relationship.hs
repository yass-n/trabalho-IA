...
main = do
	...
	let avo = Rule "avo" [e "(? x) pai (? y)", e "(?y) pai (?z)"] (e "(?x) avo (?z)")
	...
	let aula = Stream (e "joao pai jose") EmptyStream
	let aula2 =  Stream (e "jose pai augusto") EmptyStream	

	let streams = (Stream sandra
						(Stream patricia
							(Stream joaquim
								(Stream joao (Stream aula (Stream aula2 EmptyStream))))))
	...
	let rules = (Stream regra1 
					(Stream regra2 
						(Stream regra3 (Stream avo EmptyStream))))
